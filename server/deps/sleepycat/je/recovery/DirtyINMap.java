...
class DirtyINMap {
    ...
    void selectDirtyINsForCheckpoint()
        ...
        /*
         * Finish filling out the highestFlushLevels map (call
         * DbTree.getHighestLevel only when no latches are held, since it
         * latches the root), add the dirty map to the memory budget, and
         * update checkpoint state.  Synchronized to protect the state and make
         * this set of changes atomic from the evictor's viewpoint.
         */
        synchronized (this) {
    ...
    private synchronized void selectForCheckpoint(IN in) {
        ...
        if (ckptFlushAll || db.isDurableDeferredWrite()) {
            if (!highestFlushLevels.containsKey(db)) {
                ...
                /*
                 * Null is used as an indicator that getHighestLevel should
                 * be called below, when no latches are held.
                 */
                highestFlushLevels.put(db, null);
            }
        } else {
            int levelVal = level.intValue();
            if (ckptFlushExtraLevel) {
                if (in.isRoot()) {
    ...
    synchronized int getHighestFlushLevel(DatabaseImpl db) {
         ...
         * this database is should normally exist.  However, if the DB root
         * (and DatabaseImpl) have been evicted since the highestFlushLevels
         * was constructed, the new DatabaseImpl instance will not be present
         * in the map.  In this case, we do not need to checkpoint the IN and
